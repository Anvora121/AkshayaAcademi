...
// @access  Admin only
router.get('/global', verifyToken, requireRole(['admin']), async (_req: AuthRequest, res: Response) => {
    try {
        const [
            feedbackTotal,
            feedbackApproved,
            ...
            UniversityFeedback.countDocuments(),
            UniversityFeedback.countDocuments({ moderationStatus: 'approved' }),
            PlacementOffer.countDocuments(),

            // Average rating across all approved feedback
            UniversityFeedback.aggregate([
                { $match: { moderationStatus: 'approved' } },
                { $group: { _id: null, avgRating: { $avg: '$rating' } } },
            ]),

            // Placement count by country
            PlacementOffer.aggregate([
        ...
        res.json({
            ...
            placements: {
                ...
                byCountry: placementsByCountry.map((b: { _id: string; count: number }) => ({
                    country: b._id,
                    count: b.count,
                })),
                ctcByCountry: ctcByCountry.map((b: { _id: string; avgCTC: number; currency: string; count: number }) => ({
                    country: b._id,
...
// @access  Public
router.get('/university/:id', async (req, res: Response) => {
    try {
        const universityId = req.params.id;

        const [feedback, placements] = await Promise.all([
            UniversityFeedback.find({ universityId, moderationStatus: 'approved' }),
            PlacementOffer.find({ universityId }),
        ]);
