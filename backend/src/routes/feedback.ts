...
router.get('/', async (req: Request, res: Response) => {
    try {
        ...
        const feedback = await UniversityFeedback.find({
...
// @access  Admin
router.get('/pending', verifyToken, requireRole(['admin']), async (_req: AuthRequest, res: Response) => {
    try {
        const feedback = await UniversityFeedback.find({ moderationStatus: 'pending' }).sort({ createdAt: -1 });
        res.json(feedback);
    } catch {
        res.status(500).json({ message: 'Server Error' });
    }
...
// @access  Admin
router.patch('/:id/moderate', verifyToken, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
    try {
...
router.delete('/:id', verifyToken, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
    try {
        const feedback = await UniversityFeedback.findByIdAndDelete(req.params.id);
        if (!feedback) {
            return res.status(404).json({ message: 'Feedback not found' });
        }
