...
router.get('/', async (req: Request, res: Response) => {
    try {
        ...
        if (universityId) filter.universityId = String(universityId);
        if (country) filter.country = String(country);
        if (company && company !== 'all') filter.companyName = String(company);
        if (offerType) filter.offerType = String(offerType);
        ...
        const offers = await PlacementOffer.find(filter).sort({ batchYear: -1, ctcMax: -1 });
        res.json(offers);
    } catch {
...
// @access  Public
router.get('/stats/:universityId', async (req: Request, res: Response) => {
    try {
...
router.delete('/:id', verifyToken, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
    try {
        const offer = await PlacementOffer.findByIdAndDelete(req.params.id);
        if (!offer) {
